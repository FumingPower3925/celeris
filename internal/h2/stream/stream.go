...
func (m *Manager) TryOpenStream(id uint32) (*Stream, bool) {
	m.mu.Lock()
	defer m.mu.Unlock()
	// Check if stream already exists (from PRIORITY on idle stream, etc.)
	if s, exists := m.streams[id]; exists {
		// Stream exists - check if it's already active
		if s.State == StateOpen || s.State == StateHalfClosedLocal || s.State == StateHalfClosedRemote {
			// Already active, just return it
			return s, true
		}
		// Stream exists but is idle/closed - treat as new stream for concurrency check
	}
	// Check concurrency limit for new active stream
...
// GetLastClientStreamID returns the highest client-initiated stream ID observed so far,
// including streams that may have been closed and removed from the streams map.
...
func (p *Processor) ProcessFrame(ctx context.Context, frame http2.Frame) error {
	...
	if inContinuation {
		...
		if header.StreamID == expectingStreamID {
			...
			// While sending a header block on one stream, receiving HEADERS on another
			// stream is a connection error (RFC 7540 §4.3.3). Emit GOAWAY.
...
func (p *Processor) handleHeaders(_ context.Context, f *http2.HeadersFrame) error {
	...
	// we still need to open the stream so the handler can run once END_STREAM
	// arrives (h2spec CONTINUATION acceptance tests).
	if !f.HeadersEnded() {
	...
	// Note: if END_HEADERS is not set, continuation state will be established after
	// stream validation/opening below.
	if !exists {
		...
		// Update last client stream ID in a monotonic fashion
		p.manager.mu.Lock()
	...
	if f.StreamID == 0 {
		...
		if !ok {
			...
			if f.StreamID <= p.manager.GetLastClientStreamID() {
				// WINDOW_UPDATE on a closed stream is acceptable per RFC 7540 - just ignore it
				return nil
			}
		...
		// Accept WINDOW_UPDATE on closed streams but don't update window
		if stream.GetState() == StateClosed {
			// h2spec tests send WINDOW_UPDATE on closed streams - just accept it
			return nil
		}
...
func (p *Processor) handleRSTStream(f *http2.RSTStreamFrame) error {
	...
	stream.mu.Unlock()
	// Cancel any running handler to prevent writes after reset
...
func (p *Processor) sendRSTStreamAndMarkClosed(streamID uint32, code http2.ErrCode) error {
	...
	if s, ok := p.manager.GetStream(streamID); ok {
		...
		s.phase = PhaseHeadersSent // Mark as sent to prevent future writes
		s.mu.Unlock()
	}
...
func (p *Processor) handlePriority(f *http2.PriorityFrame) error {
	// Accept PRIORITY even on half-closed (remote) streams; only reject self-dependency
...
func (p *Processor) handleContinuation(_ context.Context, f *http2.ContinuationFrame) error {
	...
	if f.HeadersEnded() {
		...
		if p.continuationState.isTrailers {
			...
			stream.ReceivedInitialHeaders = true
			// Ensure ResponseWriter is set so handler can write a response
			if stream.ResponseWriter == nil {
				stream.ResponseWriter = p.connWriter
			}
			// Handler may already be running from the initial HEADERS (no END_HEADERS)
			if p.handler != nil {
				stream.mu.Lock()
